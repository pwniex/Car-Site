...
class dbConn
{
    ...
    {
        $data=array();
        $query = self::$db->query("SELECT DISTINCT marka from elektrikli ", PDO::FETCH_ASSOC);
        if($query->rowCount())
        {
    ...
    public function getModelList($marka){
        $data=array();
        $query = self::$db->prepare("SELECT DISTINCT model FROM elektrikli WHERE marka=:marka");
